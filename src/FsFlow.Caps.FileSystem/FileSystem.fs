namespace FsFlow.Caps.FileSystem
...
module FileSystem =
    ...
    let readAllText (path: string) : Flow<'env, 'e, string> when 'env :> Needs<IFileSystem> =
    ...
    let writeAllText (path: string) (contents: string) : Flow<'env, 'e, unit> when 'env :> Needs<IFileSystem> =
